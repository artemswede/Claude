...
## Статус
Готово ядро MVP (этапы 0–6):
- Онбординг с расчётом целей (TDEE) — `/start`, `/goals`
- Распознавание фото еды (Workers AI) → карточка подтверждения/правки → дневник
- Дневник и прогресс — `/today`, ручной ввод `/add`
- Контекстный чек-ин самочувствия — `/checkin`
- Советы нутрициолога с детерминированным fallback — `/advice`
- Корзина по дефицитам + партнёрский диплинк (лог `basket_click`)

Дальше (этапы 7–12): чат с НИ, скан меню/этикетки, трекинг веса, аналитика, Mini App, напоминания.
