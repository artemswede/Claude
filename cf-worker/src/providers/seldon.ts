...
export class SeldonProvider implements TenderProvider {
  ...
  private async createOrder(token: string, query: TenderQuery): Promise<string> {
    ...
    if (!res?.taskId) throw new Error("Seldon: пустой taskId от /Contracts/New");
    return res.taskId;
  }
  ...
  private mapContract(c: any): Tender {
    ...
    const currency =
      c.currency?.code ?? purchase.currency?.code ?? "RUB";
    ...
    // Победитель = поставщик контракта.
    const supplier = Array.isArray(c.suppliers) ? c.suppliers[0] : undefined;

    // Объём: сумма quantity по позициям; единица — у первой позиции.
    ...
    const volumeUnit = products[0]?.unit?.name ?? undefined;
    const okpd2 =
      products[0]?.classifier?.okpd2?.code ?? undefined;
    ...
    return {
      ...
      startPrice: money(purchase.price), // НМЦК закупки
      finalPrice: money(c.contractPrice ?? c.contractPriceRur), // цена контракта
      volume,
      volumeUnit,
  ...
  async search(query: TenderQuery): Promise<Tender[]> {
    ...
    try {
      ...
      const taskId = await this.createOrder(token, query);
      ...
      // эвристику ТБД не применяем — только цена/победитель/сортировка/лимит.
      tenders = tenders.filter((t) => Boolean(t.winner));
