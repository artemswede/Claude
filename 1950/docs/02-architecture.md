...
**Согласовано (расхождения со спекой, разрешены в пользу CJM):**
- **R1 · Вердикт 4→3 для пользователя.** Движок остаётся **4-квадрантным** (для брифа/аналитиков —
  точность), а пользователю показываем **3-цветный светофор**:
  `PoC сейчас → 🟢` · `Watchlist → 🟡` · `Классический солвер + Отбросить → ⚪ «Классики достаточно»`.
- **R2 · Анкета статичная (не адаптивная) в MVP.** Адаптивный «Акинатор» с ветвлением → v2. В MVP —
  единый набор блоков 0–6 для всех; архетип определяется ответом на «тип задачи» без архетип-зависимых
  доп.вопросов. Упрощает сборку за 1 день.
- **R3 · Длина анкеты** выровнена к «~10 вопросов».
