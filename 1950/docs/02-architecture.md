...
Self-service веб-бот с пошаговой адаптивной анкетой → детерминированный движок → вердикт + сценарий + авто-бриф.
...
- Лимиты free-tier: Workers 100k req/день, KV 1k записей/день. **Метрики батчим** — одна KV-запись
  на завершении (массив достигнутых шагов), а не на каждый шаг (фикс A11).

## 12a. Приватность и согласие (фикс A8)
- Лид-капчер (e-mail/имя) — **только опционально**, после показа ценности (бриф). Без контакта бриф
  всё равно скачивается.
- **Явный чекбокс согласия** + короткий текст: цель (передача брифа аналитикам), срок хранения
  **90 дней**, контакт для удаления. Анкета хранится **обезличенно** (без привязки к личности).
- Минимум полей; `/api/metrics` — под токен; CORS same-origin; базовый анти-спам (honeypot) на `/api/lead`.
- Это снимает противоречие «работы с данными нет»: пользовательские бизнес-данные не обрабатываем,
  храним только анонимные счётчики + опц. контакт с согласием.
...
- **R1 · Вердикт — 4 раздельных цвета**, клиент-ориентированные ярлыки (каждый даёт следующий шаг —
  клиента не бросаем, вход в нашу воронку проектов). Движок 4-квадрантный; UI:
  `PoC → 🟢 «Можно начинать пилот»` · `Watchlist → 🟡 «Перспективно — вернёмся позже»` ·
  `Классический солвер → 🔵 «Есть классическое решение»` · `Отбросить → 🔴 «Квант здесь не нужен»`.
  Все термины — простым языком, без жаргона (словарь в `01e`).
- **R2 · Анкета — финальный выбор владельца:** активны ветки **A (поиск лучшего варианта)**,
  **B (свойства материалов)** и **E (не уверен)**; ветки **C (риски)** и **D (прогноз)** показываются
  в UI как **«в разработке»** — серые неактивные кнопки. Полный набор — в `01e`.
...
## 17. AI-опция (по запросу владельца)
Технически возможно на **Cloudflare Workers AI** (без внешних интеграций, есть free-allowance). Низкорисковые
применения для MVP:
- **Ветка E «опишу своими словами»:** клиент описывает задачу текстом → ИИ предлагает тип задачи и
  предзаполняет ответы. ИИ только подсказывает — вердикт всегда считает детерминированный движок.
- **Причёсывание текста** вердикта и брифа в человеческий язык.

Риски: латентность +0.5–2 с, вариативность (движок остаётся источником истины), лимиты free-tier.
**D7 (решено):** AI — в **v1.1**. В MVP только детерминированное ядро.
